...
export default function Modal({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'relative z-10 w-full bg-white border border-navy-200 rounded-2xl animate-slide-up',
          sizeClasses[size],
          className
        )}
      >
        ...
        {!title && (
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-lg text-navy-400 hover:text-navy-700 hover:bg-navy-100 cursor-pointer transition-all duration-150 z-10"
            aria-label="Close modal"
          >
            <X className="w-4 h-4" />
          </button>
