-- 008_search_jobs_filters.sql
-- Adds role_category + country filters to search_jobs (US scope, role tags).

DROP FUNCTION IF EXISTS search_jobs(text, text[], text[], text[], text, integer, integer);
...
CREATE OR REPLACE FUNCTION public.search_jobs(
  ...
  p_role_categories text[] DEFAULT NULL,
  p_country text DEFAULT NULL
)
RETURNS TABLE(id uuid, company_id uuid, title text, normalized_title text, locations jsonb, employment_type text, remote text, salary_min numeric, salary_max numeric, tags text[], posted_at timestamp with time zone, source_url text, is_active boolean, seniority text, role_category text, country text, company_name text, company_logo text, sponsor_score numeric, rank real)
...
BEGIN
  ...
  SELECT jp.id, jp.company_id, jp.title, jp.normalized_title, jp.locations, jp.employment_type,
    ...
    jp.seniority, jp.role_category, jp.country,
    c.canonical_name AS company_name, c.logo_url AS company_logo, c.sponsor_score,
    (CASE WHEN query = '' THEN 0::real
      ELSE ts_rank(jp.search_vector, plainto_tsquery('english', query)) END
